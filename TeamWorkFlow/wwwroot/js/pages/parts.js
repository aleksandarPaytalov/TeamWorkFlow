 ...
 * Handles interactions and animations for the Parts listing page
 */
...
function initializeCardInteractions() {
    ...
    actionButtons.forEach(button => {
        ...
        button.addEventListener('mouseenter', function() {
            this.closest('.part-card').style.transform = 'translateY(-8px)';
        });
    });
}
...
function hideLoadingState() {
    ...
    if (partsGrid) {
        partsGrid.style.opacity = '1';
        partsGrid.style.pointerEvents = 'auto';
    }
}
...
window.addEventListener('error', function(e) {
    console.error('Parts page error:', e.error);
    hideLoadingState();
});
...
function initializeKeyboardNavigation() {
    document.addEventListener('keydown', function(e) {
        ...
        if (e.key === 'Enter') {
            const focusedCard = document.activeElement.closest('.part-card');
            if (focusedCard) {
...
function initializeImageLazyLoading() {
    const images = document.querySelectorAll('.part-card-image');
